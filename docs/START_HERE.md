...
每周前五天学习，周六只补缺或休息，周日验收；日志每天最多约 10 分钟，包含在学习预算里。

先保留核心能力和验收，选做顺延；W05 未通过则先重练，不叠加下周新任务。延期写入复盘，注明当前实际学习周；日期是参考。数学补缺和 AI 辅助学习按 [独立验收规则](READINESS.md) 执行。

日志按真实日期记录。面板的周时长按原日历归属，跨周补课请在日志注明实际学习周；它不会自动重排计划。
